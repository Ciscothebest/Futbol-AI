...
if (fs.existsSync(logPath)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (line.includes('const matchSearch =') || line.includes('matchSearch =')) {
      console.log(`Line ${i} contains matchSearch. Length = ${line.length}`);
      const idx = line.indexOf('matchSearch');
